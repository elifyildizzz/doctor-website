...
export default function Contact() {
  ...
  return (
    <section
      id="iletisim"
      className="relative w-full bg-[#f8f9fa] md:min-h-[calc(100vh-96px)]"
    >
                  ...
                  <div className="rounded-2xl border border-black/10 bg-white p-4 shadow-sm md:p-5">
                    ...
                    {c.ctaLabel ? (
                      <div className="mt-2 flex items-center justify-between gap-3">
                        <p className="text-sm font-medium text-[#464747]">{c.value}</p>
                        <span className="shrink-0 rounded-full bg-[#3d636d] px-3.5 py-1.5 text-xs font-semibold text-white">
                          {c.ctaLabel}
                        </span>
                      </div>
                    ) : (
                      <p className="mt-1.5 text-sm font-medium text-[#464747]">{c.value}</p>
                    )}
                  </div>
                );
